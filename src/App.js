...
function Code({ children }) {
  return (
    <div className="code-block"><pre>{children}</pre></div>
  );
}
  ...
  return (
    <motion.div {...fade}>
      ...
      <Accordion title="The core problem — why you need DNS" defaultOpen accent="#4f8ef7">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7,marginBottom:10}}>Pods are ephemeral. Every restart gives a new IP. Hardcoding IPs breaks silently when pods reschedule. You need a stable <strong style={{color:'var(--text)'}}>name</strong> — the DNS chain below is what makes that name work at runtime.</p>
      ...
      <Accordion title="Step-by-step: App → CoreDNS → kube-proxy → Pod" defaultOpen accent="#4ade80">
        <div className="chain">
          {[
            ...
            { n:4, c:'#f59e0b', t:'kube-proxy intercepts via iptables', d:<>kube-proxy watches EndpointSlices and writes DNAT rules. Traffic to <code>10.96.0.2:27017</code> gets rewritten to a real pod IP. This happens in the kernel before the packet leaves the node — invisible to the pod.</> },
   ...
   → CNI → Pod B`}</Code>
      ...
      <Accordion title="One kernel per node — not per pod" accent="#9b7ff4">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7}}>Pods are isolated network namespaces inside a single Linux kernel on each node. The kernel enforces isolation. kube-proxy's iptables rules live in the host namespace but apply to all pod namespaces. The ClusterIP rewrite is completely transparent to the pod.</p>
      </Accordion>

      <Accordion title="Service → EndpointSlice → kube-proxy — the data flow" accent="#fb923c">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7,marginBottom:10}}>When you create a Service with a selector, Kubernetes auto-creates an <strong style={{color:'var(--text)'}}>EndpointSlice</strong> listing healthy pod IPs. kube-proxy watches EndpointSlices — not pods directly. Pod dies → removed from EndpointSlice → kube-proxy updates iptables within seconds.</p>
...
function CoreDNSSection() {
  const plugins = [
    { color:'#5a5a78', name:'errors', desc:'Logs errors. Always runs first.', tag:null },
    { color:'#f59e0b', name:'health', desc:'Liveness endpoint. Goes down if can\'t reach kube API → scheduler reschedules CoreDNS. This is the OpenAI failure mode — health check fails under API load, CoreDNS restarts but needs API to restart. Circular dependency.', tag:'critical' },
    ...
    { color:'#9b7ff4', name:'kubernetes', desc:'Authoritative for .cluster.local. Watches the API via informer, maintains in-memory cache of all Services and EndpointSlices. Has sub-options: pods mode, fallthrough, ttl, endpoint_pod_names.', tag:'key' },
    ...
    { color:'#f59e0b', name:'loadbalance', desc:'Shuffles returned IPs on each headless service query. DNS-level round-robin — no proxy involved.', tag:null },
    { color:'#f87171', name:'autopath', desc:'Resolves search domain suffixes server-side (1 query instead of 5). Requires pods verified mode. Does NOT cache. Dangerous at scale — caused Datadog\'s upstream rate limit failure at 1000+ nodes.', tag:'danger' },
    { color:'#5a5a78', name:'loop', desc:'Detects infinite forwarding loops and stops them.', tag:null },
    { color:'#fb923c', name:'forward', desc:'Forwards unresolved queries upstream. Uses /etc/resolv.conf (node\'s DNS) for external domains like google.com.', tag:null },
    { color:'#5a5a78', name:'reload', desc:'Watches Corefile ConfigMap. Reloads without pod restart when changed — how you safely push config changes.', tag:null },
  ];
  const tagStyle = t => t==='key'?{background:'var(--purple-bg)',color:'var(--purple)'}:t==='danger'?{background:'var(--red-bg)',color:'var(--red)'}:t==='critical'?{background:'var(--amber-bg)',color:'var(--amber)'}:{};
    ...
    <motion.div {...fade}>
      ...
      <Accordion title="What CoreDNS deploys to your cluster" accent="#4f8ef7">
        <ul className="card-list">
          ...
          ].map(([k,v]) => <li key={k} style={{fontSize:13,color:'var(--text2)',padding:'5px 0',borderBottom:'1px solid var(--border)'}}><strong style={{color:'var(--text)'}}>{k}</strong> — {v}</li>)}
        </ul>
      </Accordion>

      <Accordion title="Plugin chain — execution order is everything" defaultOpen accent="#9b7ff4">
        <p style={{fontSize:13,color:'var(--text2)',marginBottom:12}}>Plugins execute top-to-bottom in the Corefile. Put <code>forward</code> before <code>kubernetes</code> and upstream DNS answers first — breaking cluster resolution entirely.</p>
        <div className="plugin-list">
          {plugins.map((p,i) => (
            <div className="plugin-row" key={i}>
              <div className="plugin-dot" style={{background:p.color}} />
              <div className="plugin-name">{p.name}{p.tag&&<span className="plugin-tag" style={tagStyle(p.tag)}>{p.tag}</span>}</div>
      ...
      <Accordion title="kubernetes plugin options — pods mode, fallthrough, ttl" accent="#2dd4bf">
        <div className="grid-2">
          <div>
            <p style={{fontSize:13,fontWeight:600,color:'var(--text)',marginBottom:6}}>pods [disabled|insecure|verified]</p>
            <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.6}}><strong style={{color:'var(--text)'}}>disabled</strong> (default) — pod DNS records off.<br/><strong style={{color:'var(--text)'}}>insecure</strong> — reply with IP as-given, no check.<br/><strong style={{color:'var(--text)'}}>verified</strong> — verify IP is real pod. Watches all pods. Costly in memory and API server load. Required for autopath.</p>
          ...
          <div>
            ...
            <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.6}}>Second chance: if kubernetes plugin can't answer (e.g. reverse DNS for in-addr.arpa), pass to next plugin instead of returning NXDOMAIN. Without this, reverse DNS silently fails.</p>
            <p style={{fontSize:13,fontWeight:600,color:'var(--text)',margin:'10px 0 6px'}}>ttl</p>
            <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.6}}>Override TTL of Kubernetes DNS records. Default: 5s. Lower = faster propagation, more queries. Higher = less load, slower updates.</p>
          </div>
        </div>
      </Accordion>
      ...
      <Accordion title="dnsPolicy — per pod DNS configuration" accent="#fb923c">
        <div className="plugin-list">
          ...
          ].map(p=><div className="plugin-row" key={p.n}><div className="plugin-dot" style={{background:p.c}}/><div className="plugin-name">{p.n}</div><div className="plugin-desc">{p.d}</div></div>)}
        </div>
      </Accordion>

      <Accordion title="External plugins — require custom CoreDNS build" accent="#5a5a78">
        <p style={{fontSize:13,color:'var(--text2)',marginBottom:10}}>Plugins are compiled into the CoreDNS binary — you can't add them at runtime. External plugins require a custom image build.</p>
        <div className="plugin-list">
          {[
            {c:'#9b7ff4',n:'kubernetai',d:'Single CoreDNS for multiple clusters. ClusterIPs aren\'t routable between clusters, so mostly useful for headless services with routable pod IPs.'},
            {c:'#9b7ff4',n:'rewrite',d:'Rewrite queries so legacy apps using old service names keep working across migrations.'},
            {c:'#9b7ff4',n:'static records',d:'Add override or fallback DNS records. Useful during service migration — old name resolves to same place as new name during transition period.'},
          ].map(p=><div className="plugin-row" key={p.n}><div className="plugin-dot" style={{background:p.c}}/><div className="plugin-name">{p.n}</div><div className="plugin-desc">{p.d}</div></div>)}
        </div>
      </Accordion>
    </motion.div>
...
kube-proxy → any pod`}</Code>
          ...
          <div className="compare-col" style={{borderColor:'rgba(45,212,191,0.3)'}}>
            ...
            <p style={{fontSize:13,color:'var(--text2)',marginBottom:10}}>CoreDNS returns direct pod IPs. Each StatefulSet pod gets a stable, predictable DNS name by ordinal.</p>
...
spec:
  ...
  replicas: 3`}</Code>
      ...
      <Accordion title="loadbalance plugin — DNS round-robin for headless" accent="#2dd4bf">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7}}>For headless services returning multiple pod IPs, the <code>loadbalance</code> CoreDNS plugin shuffles the order of returned IPs on each query. The client typically picks the first IP — rotating the order distributes load across pods without any proxy layer. Pure DNS-level round-robin, no kube-proxy involved.</p>
      </Accordion>
    </motion.div>
  );
}
...
function NdotsSection() {
  ...
  const dots = 'mongodb'.split('').filter(c=>c==='.').length;
  ...
  return (
    <motion.div {...fade}>
      ...
      <Accordion title="What is NDOTS and why does it matter?" defaultOpen accent="#f59e0b">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7,marginBottom:10}}>NDOTS (number of dots) is a <code>/etc/resolv.conf</code> option. If a hostname has fewer dots than the NDOTS value, the resolver tries all search domain suffixes first before querying the name as-is. Kubernetes defaults to ndots:5.</p>
        <Code>{`nameserver 10.96.0.10
search default.svc.cluster.local svc.cluster.local cluster.local
options ndots:5`}</Code>
        <p style={{fontSize:13,color:'var(--text2)',marginTop:8}}>Why 5? Because the full FQDN <code>mongodb.default.svc.cluster.local</code> has 4 dots — ndots:5 ensures it's queried directly. Sound design, unexpected production consequences at scale.</p>
      </Accordion>

      <Accordion title="Interactive: NDOTS query expansion simulator" defaultOpen accent="#f59e0b">
        <div style={{display:'flex',alignItems:'center',gap:12,margin:'0 0 16px'}}>
          <span style={{fontSize:12,color:'var(--text2)'}}>ndots value:</span>
          <input type="range" min={1} max={5} value={ndots} step={1} onChange={e=>setNdots(Number(e.target.value))} style={{flex:1}}/>
          <span style={{fontSize:16,fontWeight:700,minWidth:20,color:'var(--amber)'}}>{ndots}</span>
        </div>
        <div style={{fontSize:12,color:'var(--text2)',marginBottom:12}}>
          Query: <code style={{color:'var(--text)'}}>mongodb</code> has {dots} dot{dots!==1?'s':''}.
          {expandsSearch
            ? <span style={{color:'var(--amber)'}}> {dots} &lt; {ndots} → search domain expansion triggers ({ndots} sequential queries)</span>
            : <span style={{color:'var(--green)'}}> {dots} ≥ {ndots} → queried directly as FQDN ✓</span>}
        </div>
        <div className="ndots-list">
          {expandsSearch ? [
            {q:'mongodb.default.svc.cluster.local',r:'slow',l:'try 1 — may timeout or hit conntrack race'},
            ...
            <div className="ndots-row">
              ...
              <div className="ndots-result r-ok">queried directly — 1 query, fast ✓</div>
            </div>
          )}
        </div>
        {expandsSearch && <Alert type="warn">Each failed attempt = a UDP round trip to CoreDNS + wait for NXDOMAIN. At scale with many pods firing simultaneously, this causes the conntrack race condition.</Alert>}
      </Accordion>

      <Accordion title="The conntrack race condition — kernel-level detail" accent="#f87171">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7,marginBottom:10}}>When multiple DNS queries fire on the same UDP socket (same source port), Linux's conntrack table — which tracks UDP connections by source port — gets confused. One query stalls waiting for a 5-second timeout. Three netfilter race scenarios in Kubernetes:</p>
        <ul style={{paddingLeft:'1.2rem'}}>
          {[
            'kube-proxy replaces iptables chains while packets traverse them — brief rule mismatch window, packets dropped or misrouted',
            'Stale conntrack entries after pod deletion — pod dies, kube-proxy updates iptables, but conntrack still routes to the dead IP. This is the Preply war story.',
  ...
  - name: single-request
    value: ""          # one socket per query = no conntrack collision`}</Code>
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7,margin:'10px 0'}}>For production clusters with 50+ pods, also deploy <strong style={{color:'var(--text)'}}>NodeLocal DNSCache</strong> — a DaemonSet that runs a DNS cache on every node. Pods resolve to the local cache via link-local IP (<code>169.254.20.10</code>). Uses TCP upstream to CoreDNS, bypassing conntrack entirely. Eliminates the race condition at the architectural level.</p>
      ...
      <Accordion title="Cross-namespace resolution — the gotcha that trips everyone" accent="#9b7ff4">
        <p style={{fontSize:13,color:'var(--text2)',marginBottom:12}}>Search domains only include the pod's own namespace. From the <code>payments</code> namespace, querying <code>mongodb</code> looks for <code>mongodb.payments.svc.cluster.local</code> — not found.</p>
        <div className="grid-2">
          <div><Tag color="green">From default namespace</Tag><Code>{`mongodb          ✓ works\nmongodb.default  ✓ works\nFQDN             ✓ works`}</Code></div>
          <div><Tag color="red">From payments namespace</Tag><Code>{`mongodb          ✗ NXDOMAIN\nmongodb.default  ✓ works\nFQDN             ✓ works`}</Code></div>
        </div>
        <Alert type="info">Best practice: always use the full FQDN in multi-namespace deployments. It costs nothing and is never ambiguous.</Alert>
      </Accordion>

      <Accordion title="NXDOMAIN explained" accent="#5a5a78">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7}}>NXDOMAIN stands for "Non-Existent Domain." It's the DNS response code CoreDNS returns when a queried name doesn't exist. If you see NXDOMAIN in nslookup output, the service either doesn't exist in that namespace, you're using the wrong short name cross-namespace, or the Service hasn't been created yet.</p>
      </Accordion>

      <Accordion title="Autopath — the optimization that backfired at Datadog" accent="#f87171">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7,marginBottom:10}}>Autopath resolves search domain suffixes server-side — 1 query instead of 5. Looked great in testing. In production at Datadog's 1000+ node scale: autopath has no caching and requires <code>pods verified</code> mode (CoreDNS watches all pods — costly). This generated so many upstream queries that the upstream rate limit was reached. DNS timeouts cluster-wide. The cure was worse than the disease.</p>
        <Alert type="danger">Disable autopath at scale. Use NodeLocal DNSCache instead — it solves the same problem without the side effects.</Alert>
      </Accordion>
    </motion.div>
  );
}
  ...
  return (
    <motion.div {...fade}>
      ...
      {[
        ...
        {title:'Step 4 — check CoreDNS health', cmd:`kubectl get pods -n kube-system -l k8s-app=kube-dns\nkubectl logs -n kube-system -l k8s-app=kube-dns -f\nkubectl describe configmap coredns -n kube-system`, note:'Look for NXDOMAIN spikes, upstream timeouts, or regular IOPS spikes every N seconds (CronJob indicator — Datadog war story).'},
        {title:'Step 5 — check resolv.conf inside the pod', cmd:`kubectl exec -it <pod-name> -- cat /etc/resolv.conf`, note:'Verify nameserver is CoreDNS, check search domains match expected namespaces, confirm ndots value.'},
      ].map((s,i)=>(
      ...
      <Accordion title="Production hardening checklist" accent="#4ade80">
        <ul className="checklist">
          {['Set ndots: 2 — immediate ~80% reduction in unnecessary queries','Set single-request — eliminates conntrack race condition','CPU: minimum 200m request, 500m limit per CoreDNS pod','Use Cluster Proportional Autoscaler, not HPA','Run CoreDNS on worker nodes, not control plane nodes','Deploy NodeLocal DNSCache for clusters with 50+ pods','Monitor with Prometheus: query rate, cache hit ratio, upstream error rate','Never use autopath at scale without understanding the caching implication','Always use FQDN for cross-namespace service references','Set JVM networkaddress.cache.ttl=30 for Java apps'].map(t=><li key={t}>{t}</li>)}
        </ul>
      </Accordion>
    </motion.div>
  );
  ...
  return (
    <motion.div {...fade}>
      ...
      <Accordion title="What DNS solves vs. what it doesn't" defaultOpen accent="#4f8ef7">
        <div className="grid-2">
          <div><Tag color="green">DNS solves</Tag><ul style={{marginTop:8,paddingLeft:'1.2rem'}}>{['Where is the service? (name → IP)','Load balancing to healthy pods (via kube-proxy)','Stable endpoint across pod reschedules','StatefulSet stable identity'].map(t=><li key={t} style={{fontSize:13,color:'var(--text2)',lineHeight:1.7}}>{t}</li>)}</ul></div>
          <div><Tag color="red">DNS does NOT solve</Tag><ul style={{marginTop:8,paddingLeft:'1.2rem'}}>{['Encryption between services (mTLS)','Traffic splitting (canary deployments)','Retries, timeouts, circuit breaking','Request-level observability and tracing','Latency-aware load balancing'].map(t=><li key={t} style={{fontSize:13,color:'var(--text2)',lineHeight:1.7}}>{t}</li>)}</ul></div>
        </div>
      </Accordion>
      <Accordion title="Connect — traffic management features" accent="#4f8ef7">
        <div className="plugin-list">
          {[
            {c:'var(--blue)',n:'Latency-aware LB',d:'Routes to the fastest available pod — not random. Actively measures response times.'},
            {c:'var(--blue)',n:'Traffic shifting',d:'10% to v2, 90% to v1 without code changes. Canary deployments built in.'},
            {c:'var(--blue)',n:'Retries',d:'Failed request automatically retried on another pod. Transparent to the app.'},
            {c:'var(--blue)',n:'Circuit breaking',d:'Stop sending to overloaded/failing services before a cascade. kube-proxy has no circuit breaking.'},
          ].map(p=><div className="plugin-row" key={p.n}><div className="plugin-dot" style={{background:p.c}}/><div className="plugin-name">{p.n}</div><div className="plugin-desc">{p.d}</div></div>)}
        </div>
      </Accordion>
      <Accordion title="Secure — mTLS and zero-trust" accent="#9b7ff4">
        <div className="plugin-list">
          {[
            ...
            {c:'var(--purple)',n:'Policy enforcement',d:'Which services are allowed to talk to which. Enforced at the proxy level, not just firewall rules.'},
          ].map(p=><div className="plugin-row" key={p.n}><div className="plugin-dot" style={{background:p.c}}/><div className="plugin-name">{p.n}</div><div className="plugin-desc">{p.d}</div></div>)}
        </div>
      </Accordion>
      <Accordion title="Monitor — observability features" accent="#2dd4bf">
        <div className="plugin-list">
          {[
            ...
            {c:'var(--teal)',n:'Fault injection',d:'Deliberately inject latency or errors to test resilience. Validates your retries and circuit breakers actually work.'},
          ].map(p=><div className="plugin-row" key={p.n}><div className="plugin-dot" style={{background:p.c}}/><div className="plugin-name">{p.n}</div><div className="plugin-desc">{p.d}</div></div>)}
        </div>
      </Accordion>
      ...
      <Accordion title="How a mesh relates to DNS — they work together" accent="#5a5a78">
        <p style={{fontSize:13,color:'var(--text2)',lineHeight:1.7}}>A service mesh doesn't replace CoreDNS — it sits on top of it. Your app still calls <code>mongodb.default.svc.cluster.local</code>. DNS still resolves it. But the sidecar proxy (Envoy) intercepts the connection before it leaves the pod and applies its policies: mTLS, retries, circuit breaking, telemetry. DNS remains the discovery mechanism. The mesh adds the L7 policy layer.</p>
        <Alert type="info">Presentation tip: show <a href="https://www.youtube.com/watch?v=cjhb7_uwzDk&t=263s" target="_blank" rel="noreferrer" style={{color:'var(--blue)'}}>4:23–4:44 of this talk</a> for a great visual of the Connect/Secure/Monitor model.</Alert>
      </Accordion>
    </motion.div>
  );
}
...
function WarSection() {
  const wars = [
    {
      cls:'war-datadog',company:'Datadog · KubeCon Barcelona 2019',
    ...
    {
      cls:'war-preply',company:'Preply · Post-mortem 2020',
    ...
    {
      cls:'war-openai',company:'OpenAI · December 11 2024 · Official post-mortem',
      ...
      body:['A new telemetry service deployed to all clusters simultaneously caused every node to execute resource-intensive Kubernetes API operations — cost scaled with cluster size. API servers overwhelmed. DNS caching masked the problem initially. Once caches expired, DNS resolution failed and all services went down.','Root architectural issue: CoreDNS was running on control plane nodes. When control plane came under load, CoreDNS restarted but needed the API server to initialize — which was also down. A circular dependency.'],
    ...
    {
      cls:'war-anon',company:'Anonymous large cluster · ~2024',
      ...
      body:['CoreDNS at default settings for 18 months. Default CPU limit: 100m. At 312-pod scale, CoreDNS was throttled 97% of the time. Queries queued and timed out. All pods showed Running.','Fix: 200m request / 500m limit, ndots:2, Cluster Proportional Autoscaler, NodeLocal DNSCache.'],
  ...
  return (
    <motion.div {...fade}>
      ...
      {wars.map((w,i)=>(
        <Accordion key={i} title={`${w.company} — ${w.title}`} accent={i===0?'#9b7ff4':i===1?'#f59e0b':i===2?'#f87171':'#2dd4bf'}>
          {w.metrics&&<div className="war-metrics">{w.metrics.map(m=><div className="war-metric" key={m.lbl}><span className="val">{m.val}</span><span className="lbl">{m.lbl}</span></div>)}</div>}
    ...
    <div className="app">
      ...
      <main className="main">
        <AnimatePresence mode="wait">
          <motion.div key={active} {...fade}>{sections[active]}</motion.div>
        </AnimatePresence>
      </main>
      <footer className="footer">
        Built for Advanced Study Session · <a href="https://www.youtube.com/watch?v=QKI-JRs2RIE" target="_blank" rel="noreferrer">Datadog KubeCon 2019</a> · <a href="https://status.openai.com/incidents/ctrsv3lwd797" target="_blank" rel="noreferrer">OpenAI Post-mortem 2024</a> · <a href="https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/" target="_blank" rel="noreferrer">Kubernetes Docs</a>
      </footer>
    </div>
  );
}
